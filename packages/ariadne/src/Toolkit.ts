...
export type WithHandlerTools<T> = T extends WithHandler<infer Tools>
    ? Tools
    : never;
        ...
        return Effect.gen(this, function* () {
            ...
            const getSchemas = (tool: Tool.Any) => {
                ...
                if (Predicate.isUndefined(schemas)) {
                    ...
                    const encodeResult = Schema.encodeUnknown(
                        resultSchema,
                    ) as any;
                    schemas = {
...
export type MergeRecords<U> = {
    readonly [K in Extract<
        U extends unknown ? keyof U : never,
        string
    >]: Extract<U extends Record<K, infer V> ? V : never, Tool.Any>;
};
