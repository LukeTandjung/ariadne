...
declare module "@src/ariadne/Prompt" {
...
declare module "@src/ariadne/Response" {
            ...
            (effect, options) =>
                effect.pipe(
                    Effect.flatMap((stream) =>
                        makeStreamResponse(stream, options),
                    ),
                    Stream.unwrap,
                    Stream.map((response) => {
                        ...
                        case "file": {
                            if (part.mediaType.startsWith("image/")) {
                                ...
                                if (part.data instanceof Uint8Array) {
                                    const base64 = Encoding.encodeBase64(
                                        part.data,
                                    );
                    ...
                    switch (contentPart.type) {
                        ...
                        case "refusal": {
                            parts.push({
                                ...
                                text: "",
                                metadata: {
                                    openai: { refusal: contentPart.refusal },
                                },
                            });
                ...
                switch (event.type) {
                    case "response.created": {
                        const createdAt = new Date(
                            event.response.created_at * 1000,
                        );
                        parts.push({
                            type: "response-metadata",
                            ...
                            timestamp: DateTime.formatIso(
                                DateTime.unsafeFromDate(createdAt),
                            ),
                        });
                        break;
                    }
                    ...
                    case "response.output_item.done": {
                        switch (event.item.type) {
                            ...
                            case "function_call": {
                                ...
                                const params = yield* Effect.try({
                                    try: () =>
                                        Tool.unsafeSecureJsonParse(toolParams),
                                    catch: (cause) =>
                                        new AiError.MalformedOutput({
                                            module: "OpenAiLanguageModel",
                    ...
                    case "response.output_text.annotation.added": {
                        if (event.annotation.type === "file_citation") {
                            parts.push({
                                ...
                                mediaType: "text/plain",
                                title:
                                    event.annotation.filename ??
                                    "Untitled Document",
                                fileName:
                                    event.annotation.filename ??
                                    event.annotation.file_id,
                            });
                        }
                    ...
                    case "response.reasoning_summary_part.added": {
                        ...
                        if (event.summary_index > 0) {
                            ...
                            if (Predicate.isNotUndefined(reasoningPart)) {
                                reasoningPart.summaryParts.push(
                                    event.summary_index,
                                );
                            }
                            parts.push({
...
const annotateStreamResponse = (
    ...
    if (part.type === "finish") {
        const serviceTier = part.metadata?.openai?.serviceTier as
            | string
            | undefined;
        addGenAIAnnotations(span, {
            response: {
...
const prepareTools: (options: LanguageModel.ProviderOptions) => Effect.Effect<
    {
        readonly tools:
            | ReadonlyArray<typeof Generated.Tool.Encoded>
            | undefined;
...
> = Effect.fnUntraced(function* (options) {
    ...
    if (
        ...
        toolChoice =
            options.toolChoice.mode === "required" ? "required" : "auto";
    }
